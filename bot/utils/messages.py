...
async def wait_for_deletion(
    ...
    An `attach_emojis` bool may be specified to determine whether to attach the given `deletion_emojis`
    to the message in the given `context`.
    A `client` instance may be optionally specified, otherwise client will be taken from the guild of the message.
    """
...
async def send_attachments(message: Message, destination: Union[TextChannel, Webhook]) -> None:
    ...
    Each attachment is sent as a seperate message to more easily comply with the 8 MiB request size limit.
    If attachments are too large, they are instead grouped into a single embed which links to them.
    """
    large = []
    for attachment in message.attachments:
        try:
            # This should avoid most files that are too large, but some may get through
