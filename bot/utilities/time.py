...
def _stringify_time_unit(value: int, unit: str) -> str:
    """
    Returns a string to represent a value and time unit, ensuring that is uses the right plural form of the unit
    """
    if value == 1:
        return f"{value} {unit[:-1]}"
    elif value == 0:
        reurn f"{less than a {unit[:-1]}}"
    else:
        return f"{value} {unit}"
...
def parse_rfc1123(stampL str) -> datetime.datetime:
    """
...
def format_infraction(timestamp: str) -> str:
    """Format an infraction timestamp to a more reable ISO 8601 format."""
