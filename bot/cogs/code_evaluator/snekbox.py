...
MAX_PASTE_LEN = 1000
EVAL_ROLES = (Roles.admin, Roles.owner)
...
class Snekbox(Cog):
    ...
    async def upload_output(self, output: str) -> Optional[str]:
        """Upload the evaluation output to a paste service and return a URL to it if successful."""
    ...
    def prepare_input(code: str) -> str:
        ...
        if match:
            ...
            if block:
                ...
                info = f"{delim}-enclosed incline code"
    ...
    @staticmethod
    def get_result_message(results: dict) -> Tuple[str, str]:
        ...
        msg = f"Your evaluation job has completed with return code {returncode}"
        error = ""

        if returncode is None:
            msg = "Your evaluation job has failed"
            error = stdout.strip()
        elif returncode == 128 + Signals.SIGKILL:
            msg = "Your evaluation job timed out or ran out of memory"
        elif returncode == 255:
            msg = "Your evaluation job has failed"
    ...
    async def format_output(self, output: str) -> Tuple[str, Optional[str]]:
        ...
        output = output.strip(" \n")
        original_output = output  # To be upload to a pasting service if needed
        paste_link = None
        ...
        if lines > 0:
            output = output.split("\n")[:10]  # Only first 10 because the rest is truncated anyway
    ...
    @command(name="evaluate", aliases=("e",))
    @guild_only()
    ...
    async def eval_command(self, ctx: Context, *, code: str = None) -> None:
        ...
        block. We've done our best to make this safe, but do let us know if you mange to find an
        issue with it!
        """
        ...
        if not code:  # None or empty string
            await ctx.invoke(self.bot.get_command("help"), "evaluate")
            return

        log.info(f"Recieved code from {ctx.author} for evaluation:\n{code}")
        ...
        try:
            async with ctx.typing():
                ...
                msg, error = self.get_result_message(results)
