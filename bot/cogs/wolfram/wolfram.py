...
# Allows for 10 wolfram calls per user per day
usercd = commands.CooldownMapping.from_cooldown(Wolfram.user_limit_day, 60*60*24, BucketType.user)

# Allows for max api request / days in month per day in month per day for the entire guild (Temporary)
...
def custom_cooldown(*ignore: List[int]) -> Callable:
    ...
    async def predicate(ctx: Context) -> bool:
        ...
        if all(role.id not in ignore for role in ctx.author.roles):
            ...
            if user_rate:
                # Can't use api; because member limit
        ...
        if guild_rate:
            # Can't use api; because guild limit
            message = (
...
class Wolfram(Cog):
    ...
    async def wolfram_command(self, ctx: Context, *, query: str) -> None:
        ...
        async with ctx.channel.typing():
            ...
            if status == 501:
                message = "Failed to get repsonse"
                footer = ""
            ...
            # Sends a "blank" embed if no request is recieved, unsure how to fix
    ...
    async def wolfram_page_command(self, ctx: Context, *, query: str) -> None:
        ...
        Keywords worth nothing are, "like curve", "curve", "graph", "pokemon", etc.
        """
    ...
    async def wolfram_cut_command(self, ctx: Context, *, query: str) -> None:
        """
        Requests a drawn image of driven query.
