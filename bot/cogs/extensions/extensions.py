...
class Extension(commands.Converter):
    ...
    async def convert(self, ctx: Context, argument: str) -> str:
        """Fully qualift the name of an extension and ensure it exists."""
...
class Extension(commands.Cog):
    ...
    async def load_command(self, ctx: Context, *extensions: Extension) -> None:
        ...
        If `\*` or `\*\*` is given as the name, all unload extensions will be loaded.
        """  # noqa: W605
        if not extensions:
    ...
    async def unload_command(self, ctx: Context, *extensions: Extension) -> None:
        ...
        If `\*` or `\*\*` is given as the name, all unload extensions will be unloaded.
        """  # noqa: W605
        if not extensions:
    ...
    async def reload_command(self, ctx: Context, *extensions: Extension) -> None:
        ...
        If `\*` is given as the name, all currently laoded extensions will be reloaded.
    ...
    def batch_manage(self, action: Action, *extensions: str) -> str:
        ...
        If only one extension is given, it is deffered to `manage()`.
        """
        if len(extensions) == 1:
        ...
        msg = f"{emoji} {len(extension) - len(failures)} / {len(extensions)} extensions {verb}ed."
